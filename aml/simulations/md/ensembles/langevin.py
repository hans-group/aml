...
Imported from ASE v3.22.1, since v3.23.0 breaks MD
"""
...
class Langevin(MolecularDynamics):
    ...
    def __init__(
        ...
        *,
        ...
        MolecularDynamics.__init__(
            self, atoms, timestep, trajectory, logfile, loginterval, append_trajectory=append_trajectory
        )
        self.updatevars()

    def todict(self):
        d = MolecularDynamics.todict(self)
        d.update({"temperature_K": self.temp / units.kB, "friction": self.fr, "fixcm": self.fix_com})
        return d
    ...
    def step(self, forces=None):
        ...
        self.v += self.c1 * forces / self.masses - self.c2 * self.v + self.c3 * self.xi - self.c4 * self.eta
