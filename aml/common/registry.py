...
def _find_all_pl_callbacks():
    ...
    all_callbacks = {
        camelcase_to_snakecase(k): v for k, v in all_vars.items() if isinstance(v, type) and issubclass(v, Callback)
    }
...
def _find_all_pl_loggers():
    ...
    all_loggers = {
        camelcase_to_snakecase(k): v for k, v in all_vars.items() if isinstance(v, type) and issubclass(v, Logger)
    }
    all_loggers.pop("logger", None)
    all_loggers.pop("lightning_logger_base", None)
    all_loggers["wandb_logger"] = WandbLogger
    # fix some naming issues
    all_loggers["tensorboard_logger"] = all_loggers.pop("tensor_board_logger")
    all_loggers["mlflow_logger"] = all_loggers.pop("ml_flow_logger")
    return all_loggers
...
class Registry:
    ...
    mapping = {
        ...
        "callback_name_mapping": _find_all_pl_callbacks(),
