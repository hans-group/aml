...
from .spmm import ExplicitGradSpmm
...
def codegen_strided_tensor_product_forward(
    ...
    for ins_i, ins in enumerate(instructions):
        ...
        w3j_index.append(
            torch.cat(
                (
                    (ins_i if ins.has_weight else 0) * layout_out.base_dim  # unweighted all go in first path
                    + this_w3j_index[:, 2].unsqueeze(-1),
                    this_w3j_index[:, 0].unsqueeze(-1) * layout_in2.base_dim + this_w3j_index[:, 1].unsqueeze(-1),
                ),
                dim=1,
            )
        )
    ...
    w3j_i_indexes = torch.div(w3j.indices()[1], layout_in1.base_dim, rounding_mode="floor")
    w3j_j_indexes = w3j.indices()[1] % layout_in1.base_dim
    w3j_is_ij_diagonal = (layout_in1.base_dim == layout_in2.base_dim) and torch.all(w3j_i_indexes == w3j_j_indexes)
    ...
    if sparse_mode is None:
        ...
        w3j = w3j.to_dense().reshape(((num_paths,) if num_paths > 1 else tuple()) + kij_shape).contiguous()
        del kij_shape
            ...
            contracted = Proxy(
                graph_out.call_module(
                    ...
                    (
                        outer.reshape(
                            -1,
                            (layout_in1.base_dim if w3j_is_ij_diagonal else layout_in1.base_dim * layout_in2.base_dim),
                        ).T.node,
                    ),
                )
            ).T.reshape(
                (-1,)
                + {"uv": (layout_in1.mul, layout_in2.mul), "uu": (layout_in1.mul,)}[connection_mode[:2]]
                + (num_paths, layout_out.base_dim)
            )
