    ...
    def __init__(self, irreps: Irreps, pad_to_multiple: int = 1):
        ...
        for mul_i in range(self.mul):
            for irrep_i, (_, irrep) in enumerate(self.base_irreps):
                ...
                catted_indexes = torch.arange(irrep.dim) + self.irreps[:irrep_i].dim + irrep.dim * mul_i
        ...
        assert torch.all(self.indexes_to_strided[self.indexes_to_catted] == torch.arange(self.irreps.dim))
