...
def codegen_strided_linear(
    ...
    ins_per_output: List[List[Instruction]] = [
        [ins for ins in instructions if ins.i_out == i] for i in range(len(layout_out.base_irreps))
    ]
    ...
    for ins_group in ins_per_output:
        ...
        assert all(
            layout_in.base_irreps[min_i_in] == layout_in.base_irreps[i] for i in range(min_i_in, max_i_in + 1)
    ...
    for ins_grp_i, (ins_grp, ins_grp_ins) in enumerate(zip(ins_per_output, ins_group_irrep_slice)):
        ...
        to_mix = x[
            :,
            :,
            layout_in.base_irreps[: ins_grp_ins[0]].dim : layout_in.base_irreps[: ins_grp_ins[1] + 1].dim,
        ...
        to_mix = to_mix.reshape(-1, layout_in.mul, n, layout_in.base_irreps[ins_grp_ins[0]].ir.dim)  # z u n i
        n_weight = layout_in.mul * n * layout_out.mul
        if shared_weights:
            this_w = ws[w_index : w_index + n_weight].reshape(layout_out.mul, layout_in.mul, n)
        else:
            this_w = ws[:, w_index : w_index + n_weight].reshape(-1, layout_out.mul, layout_in.mul, n)
    ...
    outs = [
        ((1.0 / sqrt(sum(layout_in.mul for ins in instructions if ins.i_out == i))) * out)
    ...
    graphmod_out = fx.GraphModule(constants_root, graph_out, class_name="linear_forward")
...
def Linear(
    ...
    instructions = [Instruction(i_in=e[0], i_out=e[1], path_shape=None) for e in instructions]
