...
class ScalarMLP(GraphModuleMixin, torch.nn.Module):
    ...
    def __init__(
        ...
        self.irreps_out[self.out_field] = o3.Irreps([(self._module.out_features, (0, 1))])
...
class ScalarMLPFunction(CodeGenMixin, torch.nn.Module):
    ...
    def __init__(
        ...
        for layer, (h_in, h_out) in enumerate(zip(dimensions, dimensions[1:], strict=False)):
            # do dropout
            if mlp_dropout_p > 0:
            ...
            if mlp_initialization == "normal":
                ...
                raise NotImplementedError(f"Invalid mlp_initialization {mlp_initialization}")
