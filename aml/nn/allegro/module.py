...
from ..nequip.utils import tp_path_exists
from .cutoffs import cosine_cutoff, polynomial_cutoff
from .fc import ScalarMLPFunction
from .strided import Contracter, Linear, MakeWeightedChannels
...
class Allegro_Module(GraphModuleMixin, torch.nn.Module):
    ...
    def __init__(
        ...
        node_invariant_field: str = "node_attr",
        ...
        latent_out_field: Optional[str] = K.edge_features,
        ...
        for layer_idx, (arg_irreps, out_irreps) in enumerate(zip(tps_irreps_in, tps_irreps_out)):
            ...
            tp = Contracter(
                irreps_in1=o3.Irreps(
                    [
                        (
                            (env_embed_multiplicity if layer_idx > 0 or self.embed_initial_edge else 1),
                            ir,
                        )
        ...
        if latent_resnet_update_ratios is None:
            ...
            latent_resnet_update_ratios = torch.as_tensor(latent_resnet_update_ratios, dtype=torch.get_default_dtype())
            assert latent_resnet_update_ratios.min() > 0.0
            assert latent_resnet_update_ratios.min() < 1.0
            latent_resnet_update_params = torch.special.logit(latent_resnet_update_ratios)
        ...
        if latent_resnet_update_ratios_learnable:
            self._latent_resnet_update_params = torch.nn.Parameter(latent_resnet_update_params)
        else:
            self.register_buffer("_latent_resnet_update_params", latent_resnet_update_params)
        ...
        self.irreps_out.update(
            {
                self.latent_out_field: o3.Irreps([(self.final_latent.out_features, (0, 1))]),
            }
        )
    ...
    def forward(self, data: DataDict) -> DataDict:
        ...
        :return: DataDict
        """
        edge_center = data[K.edge_index][1]
        edge_neighbor = data[K.edge_index][0]
        ...
        edge_length = torch.linalg.norm(data[K.edge_vec], dim=-1)
        ...
        for latent, env_embed_mlp, env_linear, tp, linear in zip(
            ...
            if self.embed_initial_edge and layer_index == 0:
                ...
                features = self._env_weighter(features[prev_mask], env_w)  # features is edge_attr
            else:
            ...
            if self.env_sum_normalizations.ndim < 2:
                ...
                norm_const = self.env_sum_normalizations[layer_index, data[AK.elem_map]].unsqueeze(-1)
        ...
        prev_mask = cutoff_coeffs[active_edges] > 0
        active_edges = (cutoff_coeffs > 0).nonzero().squeeze(-1)
        new_latents = self.final_latent(torch.cat(latent_inputs_to_cat, dim=-1)[prev_mask])
        new_latents = cutoff_coeffs[active_edges].unsqueeze(-1) * new_latents
